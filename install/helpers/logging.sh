...
start_install_log() {
  ...
  echo "=== Airchy Installation Started: $OMARCHY_START_TIME ===" >>"$OMARCHY_INSTALL_LOG_FILE"
  start_log_output
}
...
stop_install_log() {
  ...
  if [[ -n ${OMARCHY_INSTALL_LOG_FILE:-} ]]; then
    ...
    echo "=== Airchy Installation Completed: $OMARCHY_END_TIME ===" >>"$OMARCHY_INSTALL_LOG_FILE"
    ...
    if [ -n "$OMARCHY_START_TIME" ]; then
      ...
      echo "Airchy:     ${OMARCHY_MINS}m ${OMARCHY_SECS}s" >>"$OMARCHY_INSTALL_LOG_FILE"
